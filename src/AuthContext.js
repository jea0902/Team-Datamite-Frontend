...
// export const AuthContext = createContext({ isLogin: token !== null ? true : false})
...
export function IsLoginProvider({ children }) {
    ...
    // useMemo로 캐싱하지 않으면 value가 바뀔 때마다 state를 사용하는 모든 컴포넌트가 매번 리렌더링됨
    const value = useMemo(() => ({ isLogin, setIsLogin, userData, setUserData }), [isLogin, setIsLogin, userData, setUserData])
    return <AuthContext.Provider value={value}>{children}</AuthContext.Provider>
}
