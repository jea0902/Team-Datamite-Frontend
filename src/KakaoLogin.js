...
function KakaoLogin() {
  ...
  const sendAuthCodeToBackend = (authCode) => {
    axios.post("백엔드 주소", {authCode : authCode})
    .then(response => {
    ...
    axios.post("백엔드 주소", { token: token})
      ...
      .catch(error=> {
        ...
        if (error.response && error.response.data) {
          console.error("서버로부터 에러 메시지 :",error.response.data.message);
        }
      });
  }
  ...
  return (
    <div>
      <button
        ...
        style={{ display: "flex", width: "100%", marginBottom: "2vh", backgroundColor: "#f9e000", color: "black", fontSize: "16px" }}
        onClick={loginWithKakao}
      >
        <img src={카카오로고} alt="logo" className='kakao-image' />
        <div style={{ alignItems: "center" }}>카카오 간편 로그인</div>                         
      </button>
      {kakaoToken && <p id="token-result">login success, token: {kakaoToken}</p>}
    </div>
  );
}
