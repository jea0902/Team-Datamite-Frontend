  ...
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(async function (position) {
        ...
        // 마커랑 인포윈도우 생성 함수.
        const createMarkerWithInfoWindow = (position, content, hospital = null) => {
          ...
          if (hospital) {
            markers.push(marker);
            const categoryName = hospital.category_name.split('>').pop().trim();
            ...
            const customInfoWindow = new window.kakao.maps.CustomOverlay({
              ...
              <div style="max-width:400px; overflow-wrap:break-word; padding:8px; background-color:white; border-radius: 5px; border: 1px solid #ccc; font-size: 12px;">
                <div style = "margin-right : 25px;">
                  ...
                  <span style="color:green;">${hospital.phone || "전화번호 정보 없음"}</span><br />
        ...
        const loadHospitals = async (lat, lon) => { // 인자로 전달받은 lat과 lon을 사용하도록 변경 - 함수 내에서 직접적으로 lat,lon 받게 하지 말고

          // 기존의 마커 제거 (<= 반경 변경될 떄마다 초기화 시켜줘야 다시 불러왔을 때 불러와지기에)
          markers.forEach(marker => marker.setMap(null));
          markers.length = 0; // 마커 배열 초기화

          if (clustererRef.current) { // 기존의 클러스터 제거 (<= 반경 변경될 떄마다 초기화 시켜줘야 다시 불러왔을 때 불러와지기에)
          ...
          clustererRef.current = new window.kakao.maps.MarkerClusterer({ // clusterer 객체에 저장해야 나중에 초기화 가능.
            map: map, // 지도 객체
          ...
          function getTexts(count) { return count + "개"; }

          // const hospitals = await fetchHospitalsFromKakao(selectedDepartment, lat, lon, radius);
          const hospitals = await fetchHospitalsFromKakao(resultSpeciality, lat, lon, radius);
          ...
          hospitals.forEach(hospital => {
            const hospitalPosition = new window.kakao.maps.LatLng(hospital.y, hospital.x);
            const createMarker = createMarkerWithInfoWindow(hospitalPosition, hospital.place_name, hospital);
